...
describe('Traverse: complete task barrier', () => {
  test('complete-task-marks-done', async () => {
    ...
    const res = await seneca.post('sys:traverse,on:task,do:complete', { task })
  ...
  test('complete-task-last-task-completes-run', async () => {
    ...
    await seneca.entity('sys/traverse').load$(run.id).then((r: any) => {
      r.status = 'active'
      return r.save$()
    })
  ...
  test('complete-task-did-complete-not-fired-when-not-last', async () => {
    ...
    await seneca.post('sys:traverse,on:task,do:complete', { task: tasks[1] })
  ...
  test('async-mode-completion-via-barrier', async () => {
    ...
    // Override dispatch to NOT call do:complete (worker does it via task_msg)
    seneca.message(
